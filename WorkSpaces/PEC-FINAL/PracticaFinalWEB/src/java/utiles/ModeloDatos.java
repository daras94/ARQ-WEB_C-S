...
public class ModeloDatos {
    ...
    public void obtenerBilletes(String o, String d,String fi, String fv){
        try {
            ...
            java.util.Date date_ida = formatter.parse(fi);  //pasamos la fecha de ida a un formato entendible por la bbdd
            java.util.Date date_vuelta = formatter.parse(fv);
            ...
            resultado = mandato.executeQuery("SELECT * FROM public.trayecto where plazas > 0 and aer_origen = '%" + o + "%' and aer_destino = '%" + d + "%' and timestamp_column::fecha = date '%" + date_ida+ "%'" + "%' and timestamp_column::fecha = date '%" + date_vuelta+ "%' order by fecha");
        }
         catch (SQLException ex) {
            Logger.getLogger(ModeloDatos.class.getName()).log(Level.SEVERE, null, ex);
        } catch (Exception e) {
